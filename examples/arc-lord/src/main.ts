...
// Pacing — same constants as the recipes so the Arc Lord example is
// directly comparable.
...
function pickWeighted(): string {
  const r = Math.random();
  // 0 is the rare premium; 1-5 are commons; 6-9 are mids.
  if (r < 0.03) return '0';
  if (r < 0.18) return SYMBOL_IDS[6 + Math.floor(Math.random() * 4)]!;  // 6-9
  return SYMBOL_IDS[1 + Math.floor(Math.random() * 5)]!;                 // 1-5
}
...
// ------------------------------------------------------------
// WIN DETECTION — left-to-right runs of the same id (wilds tolerant)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// WIN DESTRUCTION (synced to destroy_highlight SFX)
// ------------------------------------------------------------
...
async function main(): Promise<void> {
  ...
  await app.init({ background: 0x0a0518, resizeTo: window, antialias: true });
  document.body.appendChild(app.canvas);

  gsap.ticker.remove(gsap.updateRoot);
  app.ticker.add(() => gsap.updateRoot(app.ticker.lastTime / 1000));

  // Load spine + audio in parallel; both must complete before the first spin.
  await Promise.all([loadArcLordSpines(), loadAllSounds()]);
  ...
  const reelSet = new ReelSetBuilder()
    ...
    .symbolGap(GAP, GAP)
    .symbols((r) => {
      // Arc Lord spines are authored ~220–260 px; 0.45 fits them into a
      // 110 px cell with a small breathing margin around the art.
  ...
  // Layout the reel set in the centre.
  const totalW = REELS * (CELL + GAP) - GAP;
  const totalH = ROWS * (CELL + GAP) - GAP;
  const wrapper = new Container();
  wrapper.addChild(reelSet);

  // A faint gold frame around the reels — gives the spine art a stage.
  ...
  // ── SPIN BUTTON ──────────────────────────────────────────────
  // Drawn as a Pixi Graphic so the demo is self-contained — no DOM
  // overlay button to keep in sync.
  ...
  btnBg.circle(0, 0, 36);
  btnBg.fill({ color: 0xffd166 });
  btnBg.circle(0, 0, 36);
  btnBg.stroke({ color: 0xfff4cc, width: 2 });
  ...
  function positionButton(): void {
    ...
    spinBtn.y = app.screen.height - 56;
  }
  positionButton();
  ...
  spinBtn.on('pointerdown', () => {
    if (!unlocked) {
      // First click counts as the audio unlock (iOS gesture quota).
  ...
  function refreshHud(): void {
    winEl.textContent = totalWin.toFixed(0);
  ...
  async function handleSpin(): Promise<void> {
    ...
    // Moment A — initial spin (LTR reveal).
