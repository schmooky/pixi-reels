...
Each SPIN click plays the **full cascade sequence**:
...
async function spin() {
  ...
  reelSet.setResult(response.stages[0].map((visible) => ({ visible })));
  await p;
...
const p = reelSet.spin();
reelSet.setResult(first.map((visible) => ({ visible })));
await p;
...
- **Per-cascade options**. pass `destroyOptions: { delay, zIndex, dim }` to `runCascade`.
- **Replace entirely**. drive the visual yourself inside `onCascade`, then pass `destroyOptions: { zIndex: null }` so the engine's implode is invisible. See [Cascade wins. present, destroy, refill](/recipes/cascade-destroy-and-present/).

### 4. Identifying winners

The canonical winner shape is `Cell[]`. `{ reel, cell }` for every cell your match rules consider a winner. Compute it from the grid your `detectWinners` callback receives; never from a naive diff of "the next grid differs at row X." That bug only surfaces when survivors slide past cleared rows, and is the most common cascade implementation mistake.
...
const offsets = computeDropOffsets(5 /* visibleCells */, [1, 3] /* winnerCells */);
// Two winners, so every NEW symbol falls the same 2 slots -- their virtual
// origins are stacked above the viewport, not spread one per cell.
expect(offsets.find((o) => o.cell === 0)?.offsetCells).toBe(2);
expect(offsets.find((o) => o.cell === 4)?.offsetCells).toBe(0); // survivor, no move
```

For end-to-end tests that drive a real `ReelSet`, use the [testing utilities](/guides/cheats-and-testing/). `createTestReelSet`, `FakeTicker`, `spinAndLand`, `captureEvents`.
