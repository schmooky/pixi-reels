...
- **Replace entirely**. drive the visual yourself inside `onCascade`, then pass `destroyOptions: { zIndex: null }` so the engine's implode is invisible. See [Cascade destroys. fade, explode, present](/recipes/cascade-destroy-and-present/).
