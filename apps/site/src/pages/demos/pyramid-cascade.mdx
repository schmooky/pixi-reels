---
layout: ../../layouts/Demo.astro
title: Pyramid cascade (3-5-5-5-3)
subtitle: Jagged pyramid - ways pay - gravity refill (survivors stay, only gaps drop)
tags: [pyramid, cascade, ways, gravity]
description: A diamond-shaped 3-5-5-5-3 reel set built with visibleCellsPerReel + center anchor. Cascade refills are real gravity - survivors fall only as far as the cleared slots beneath them, symbols with no cleared slots below them don't move at all.
---

import PyramidCascadeDemo from '../../components/demos/PyramidCascadeDemo.tsx';

<PyramidCascadeDemo client:only="react" />

## The mechanic

```ts
new ReelSetBuilder()
  .reels(5)
  .visibleCellsPerReel([3, 5, 5, 5, 3])
  .reelAnchor('center')   // diamond shape
  .tumble({
    fall:   { duration: 280, ease: 'power3.in',  cellStagger: 60 },
    dropIn: { duration: 450, ease: 'power3.out', cellStagger: 60, distance: 'perHole' },
  })
```

A stiff `power3.out` drop-in (no overshoot) keeps the symbols landing hard. the bouncy default would fight the win animation that arrives a frame later.

## Gravity refill (not "redrop the whole frame")

A naive cascade phase would fall **all** old symbols out then drop **all** new ones in. That's wrong for a tumble: survivors should stay put, only the gaps left by winners should refill from above.

The fix is the canonical cascade pair, `reelSet.destroySymbols(winners)` + `reelSet.refill({ winners, grid })`:

```ts
await reelSet.destroySymbols(winnerCells);                  // pop animation
const next = computeRefillGrid(grid, winnerCells);          // gravity-correct grid (string[][])
await reelSet.refill({                                       // survivors slide; new arrive
  winners: winnerCells,
  grid: next.map((visible) => ({ visible })),               // wrap as ColumnTarget[]
});
```

Per column the engine computes per-cell offsets:

- New symbol filling cleared cell R -> starts (W - R) slots above the viewport,
  where W is that reel's winner count. The virtual origins are stacked, so every
  new symbol falls exactly W slots.
- Survivor at final row R, originally at row N -> falls (R - N) slots.
- A symbol whose `R === N` doesn't move at all.

So a 5-cell reel with a single winner at cell 0 only animates one symbol. the new top. Everything below stays put. Jagged shapes work the same way; each reel's `visibleCells` is read at refill time.

## Ways evaluation on a jagged grid

Same algorithm as MultiWays: walk reels left-to-right counting reels that contain the kind (or wild). The pyramid's 3-row outer reels have fewer cells per reel. the chain logic doesn't care:

```ts
for (let c = 0; c < REEL_COUNT; c++) {
  const matches = grid[c].flatMap((s, r) =>
    s === kind || s === 'wild' ? [{ reelIndex: c, cellIndex: r }] : []);
  if (matches.length === 0) break;
  cellsByReel.push(matches);
}
```

## Round flow

1. `roundBus.emit('round:reset')`. WinBox to 0.
2. Initial `reelSet.spin()` → `setDropOrder('ltr')` → `setResult(grid)`.
3. `reelSet.runCascade({ detectWinners, nextGrid })` loops the chain. the awaited promise resolves with `RunCascadeResult` when it ends.
4. Inside `onCascade`: `roundBus.emit('win:add', cascadeWin)`. WinBox additively tickups.
5. Status shows `N CASCADES. TOTAL`.
