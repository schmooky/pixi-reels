...
```ts
import { loadPrototypeSymbols } from 'examples/shared/prototypeSpriteLoader';
...
```ts
import { BlurSpriteSymbol } from 'examples/shared/BlurSpriteSymbol';
...
Missing blur entries fall through to the base texture. You can ship blur for a subset of symbols and plain textures for the rest without a second code path.
