...
const { reelSet, spinAndLand } = createTestReelSet({
  reels: 5, visibleCells: 3, symbolIds: ['a','b','c','scatter'],
});
const engine = new CheatEngine({ reelCount: 5, visibleCells: 3, symbolIds: ['a','b','c','scatter'], seed: 1 });
...
- **Force 4 scatters**. 20 free spins instead of 15. there are no mid-FS retriggers.
