...
const reelSet = new ReelSetBuilder()
  .reels(5).visibleCells(3).symbolSize(96, 96)
  .symbols((r) => {
...
```ts
import { createTestReelSet } from 'pixi-reels/testing';
import { CheatEngine, forceLine } from 'examples/shared/cheats';

const { reelSet, spinAndLand, destroy } = createTestReelSet({
  reels: 5, visibleCells: 3, symbolIds: ['cherry','seven','wild'],
});
const engine = new CheatEngine({ reelCount: 5, visibleCells: 3, symbolIds: ['cherry','seven','wild'], seed: 1 });
