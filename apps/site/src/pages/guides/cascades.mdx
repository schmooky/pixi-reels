...
|---|---|---|
| `cascade:round:start` | `runCascade()` opens | Lock auto-play, fade in cascade music bus, log a round-start to analytics. |
...
| `cascade:round:end` | `runCascade()` exits | Unlock UI, show "big win" overlay if `chainLength > 2`, fade out music. Carries `RunCascadeResult`. |
...
…or call **`reelSet.runCascade({ detectWinners, nextGrid })`**, which is the same loop with `cascade:round:start` fired on entry and `cascade:round:end` (carrying the summary) at the end. The two callbacks may be `async` — `nextGrid` is the natural place to `await server.cascade(winners)`.
...
- Fires `cascade:round:end` with `wasSkipped: true`.
