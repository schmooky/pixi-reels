...
Round- and chain-scoped events live alongside the per-phase ones, so a HUD or audio bus can sit on a single bus:

| Event | When | What it's for |
|---|---|---|
| `cascade:roundStart` | `runCascade()` opens | Lock auto-play, fade in cascade music bus, log a round-start to analytics. |
...
| `cascade:complete` | `runCascade()` exits | Unlock UI, show "big win" overlay if `chainLength > 2`, fade out music. |

`runCascade` owns these events — if you compose your own loop with bare `refill()` calls, only the phase-level ones fire.
...
…or call **`reelSet.runCascade({ detectWinners, nextGrid })`**, which is the same loop with `cascade:complete` fired at the end. The two callbacks may be `async` — `nextGrid` is the natural place to `await server.cascade(winners)`.
...
`reelSet.skip()` slams the in-flight phase but **early-returns when the engine is idle** (the moment between two refills, or during the `pauseAfterDestroyMs` beat). That means a tap to slam can land in a window where `skip()` does nothing — and your cascade chain keeps running.
...
- If an `refill()` is in flight, calls `slamStop()` so the await unblocks immediately.
- Exits at the next await boundary.
- Fires `cascade:complete` with `wasSkipped: true`.

This is the canonical "player slam" pattern. The standard `reelSet.skip()` is still the right tool inside event listeners or while the engine is actively in motion — both work together.
...
2. **Want to decorate new symbols after they land (multiplier badges)?** Subscribe to `cascade:place:done`. Use `isInitial` / `winnerRows` in the payload to skip survivors.
