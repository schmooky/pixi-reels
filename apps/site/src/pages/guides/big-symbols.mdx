...
Pin overlays render at zIndex `10000`. So a big symbol that's also pinned (`reelSet.pin(anchorCol, anchorRow, id, ...)`) gets its overlay rendered above the cell — the overlay covers the whole block visually because the overlay symbol is sized to the block.
