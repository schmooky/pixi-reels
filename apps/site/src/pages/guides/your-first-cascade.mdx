...
- `reelSet.skipSpin()` slams the engine's in-flight motion. If the engine is idle (between refills, during the `pauseAfterDestroyMs` beat) it's a no-op — that's safe to call unconditionally.
