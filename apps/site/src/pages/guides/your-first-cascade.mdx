...
1. Fires `cascade:round:start` once.
2. Calls `detectWinners(grid)` with the current visible grid.
3. If empty, ends the chain and fires `cascade:round:end` with the round summary.
4. Otherwise calls `reelSet.destroySymbols(winners)`, then `nextGrid(grid, winners)`, then `reelSet.refill(...)`.
5. Loops back to step 2 with the post-refill grid.
