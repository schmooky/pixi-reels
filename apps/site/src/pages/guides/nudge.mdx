...
Where `total = bufferAbove + visibleRows + bufferBelow`. Use case: a 1xH
wild lands with stubs in bufferBelow; nudge up to reveal the full block
in the visible window. Cross-reel blocks (w > 1) always throw — splitting
an anchor from its right-side cells on another reel isn't safe.
