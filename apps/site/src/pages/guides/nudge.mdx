...
import sequentialCode from '../../recipes/nudge-sequential.recipe.ts?raw';
import parallelCode from '../../recipes/nudge-parallel.recipe.ts?raw';
...
<RecipeRunner code={sequentialCode} client:only="react" />
...
<RecipeRunner code={parallelCode} client:only="react" />
...
its post-rotation position keeps the whole block on the strip AND the
anchor lands in visible (for up):

| Direction | Survival condition |
| --- | --- |
| `'down'` | `anchorStripIdx + h - 1 + distance < total` |
| `'up'`   | `anchorStripIdx - distance >= bufferAbove` |
...
Down can land the block half-clipped into bufferBelow (the anchor stays
in visible; the stub spills below the mask — renders fine). Up must
keep the anchor in visible because `_finalizeFrame` doesn't size
anchors that sit above visible today. Use case: a 1xH wild lands with
stubs in bufferBelow; nudge up to reveal the full block in the visible
window. Cross-reel blocks (w > 1) always throw — splitting an anchor
from its right-side cells on another reel isn't safe.
