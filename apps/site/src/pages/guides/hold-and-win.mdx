...
3. **Fork the board.** Want lock-and-respin but with *different* rules? Copy
   `HoldAndWinBoard` + `HoldAndWinState` into your project — they touch only public
   `pixi-reels` API, so the copy stays on supported ground — and rewrite the
   reducer. The respin-reset-on-hit rule and board-full-ends-the-feature rule live
   in `HoldAndWinState`, yours to change.
