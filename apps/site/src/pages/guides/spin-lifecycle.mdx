...
button.addEventListener('click', () => {
  if (reelSet.isSpinning) {
    ...
    // slams. skip() does nothing useful when called pre-result.
    reelSet.requestSkip();
  } else {
    reelSet.spin();
  }
...
`skip()` / `requestSkip()` both force-land on whatever `setResult()` told the
engine. `result.wasSkipped === true`. The lifecycle hooks (`spin:reelLanded`,
`spin:allLanded`, `spin:complete`) all still fire on the slam path — so win
presenters and effect chains keep working without a separate code path.
...
const reelSet = new ReelSetBuilder()
  // ...
  .cascade(DropRecipes.stiffDrop) // register cascade phases
  .ticker(app.ticker)
  .build();
...
`.cascade(...)` on the builder — the error names the missing method.
...
| `skip:requested` | — | `skip()` / `requestSkip()` actually fires the slam |
