...
| Cascade | `cascade:round:start` · `cascade:round:end` · `cascade:chain:start` · `cascade:chain:end` · `cascade:fall:start` · `cascade:fall:symbol` · `cascade:fall:end` · `cascade:place:end` · `cascade:dropIn:start` · `cascade:dropIn:symbol` · `cascade:dropIn:end` · `cascade:destroy:start` · `cascade:destroy:end` | `.tumble()` + `spin({mode:'cascade'})` / `refill()` / `runCascade()` / `destroySymbols()` |
...
Every cascade event uses the same three-part shape: `cascade:<scope>:<step>`. Scopes are `round` (the runCascade lifecycle), `chain` (one stage inside a chain), `fall` / `place` / `dropIn` (phase-level animation), `destroy` (one batch of destroySymbols). Steps are `start` / `symbol` / `end` — `:symbol` appears only on the per-cell variants of fall and dropIn. `:place` has only `:end` because placement is a single-moment swap with no animation.
...
### Round-scoped (runCascade lifecycle)

| Event | Payload | When |
|---|---|---|
| `cascade:round:start` | `{ initialGrid: string[][] }` | Fired once at the top of `runCascade(...)` — the round just opened, before the first `detectWinners`. Use to lock auto-play, fade in a cascade music bus, log a round-start. **Not** emitted when you hand-roll a loop with bare `refill()` calls. |
| `cascade:round:end` | `info: RunCascadeResult` | Mirror of `cascade:round:start`. Fired once after the detect → destroy → refill loop exits (clean end, `maxChain` hit, or slam). Payload is the same `{ chainLength, totalWinners, finalGrid, wasSkipped }` shape returned by `runCascade()`. |
...
reelSet.events.on('cascade:round:start', () => hud.lock());
reelSet.events.on('cascade:round:end',   () => hud.unlock());
