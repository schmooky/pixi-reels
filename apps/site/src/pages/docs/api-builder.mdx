...
| `.visibleRows(n)` or `.visibleRowsPerReel([n,...])` or `.multiways({...})` | One of these picks the row layout. `visibleSymbols(n)` is the legacy alias of `visibleRows(n)`. |
...
| `.initialFrame(string[][] \| ColumnTarget[])` | Random per `weights()`. Use `ColumnTarget[]` for any grid that crosses worker / JSON boundaries. |
