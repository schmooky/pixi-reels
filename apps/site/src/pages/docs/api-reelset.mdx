...
| Method | Signature | Notes |
|---|---|---|
| `refill` | `(opts: { winners: ReadonlyArray<Cell>; grid: string[][] \| ColumnTarget[] }) => Promise<SpinResult>` | Moment B: place + drop-in for survivors and new symbols. Skip the fall and the wait-for-result entirely. Throws without `.tumble()`. |
| `destroySymbols` | `(cells: ReadonlyArray<Cell>, opts?: DestroySymbolsOptions) => Promise<void>` | Defer to each symbol's `playDestroy()`. Lifts zIndex so destroys aren't clipped; alternates rotation by column. Optional `direction`, `delay`, `zIndex`, `dim`. |
| `runCascade` | `(opts: RunCascadeOptions) => Promise<RunCascadeResult>` | One-call cascade orchestration. Loops detect → destroy → pause → refill. Pass `signal: AbortSignal` for caller-driven cancellation (use this for "player tapped slam between refills" — `reelSet.skip()` is a no-op when the engine is idle). Emits `cascade:round:start` on entry and `cascade:round:end` on exit. |
...
| `RunCascadeOptions` | `{ detectWinners, nextGrid, onCascade?, pauseAfterDestroyMs?, maxChain?, destroyOptions?, signal? }` | `detectWinners` and `nextGrid` carry the game rules; everything else is timing / cancellation. Both callbacks may be `async`. Defaults: `pauseAfterDestroyMs: 250`, `maxChain: 32`. `onCascade` payload is `{ chain, winners, currentGrid }` — same grid that `cascade:chain:start` reported. |
| `RunCascadeResult` | `{ chainLength, totalWinners, finalGrid, wasSkipped }` | Returned by `runCascade` AND delivered to `cascade:round:end` listeners. `chainLength === 0` means no wins on the initial grid (no refill ran). |
