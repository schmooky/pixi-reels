...
| Method | Signature | Notes |
|---|---|---|
| `refill` | `(opts: { winners: Cell[]; grid: string[][] \| ColumnTarget[] }) => Promise<SpinResult>` | Moment B: place + drop-in for survivors and new symbols. Skip the fall and the wait-for-result entirely. Throws without `.tumble()`. |
| `destroySymbols` | `(cells: readonly Cell[], opts?: DestroySymbolsOptions) => Promise<void>` | Defer to each symbol's `playDestroy()`. Lifts zIndex so destroys aren't clipped; alternates rotation by column. Optional `direction`, `delay`, `dim`. |
...
| `DestroySymbolsOptions` | `{ direction?, delay?, zIndex?, dim? }` | Defaults: alternating direction by column, zero delay, zIndex `1000`, no viewport dim. Each field accepts either a value OR a `(cell, index) => value` function for per-cell control. Pass `zIndex: null` to suppress the bump (useful when another presenter already drove the visual). |
| `RunCascadeOptions` | `{ detectWinners, nextGrid, onCascade?, pauseAfterDestroyMs?, maxChain?, destroyOptions?, signal? }` | `detectWinners` and `nextGrid` carry the game rules; everything else is timing / cancellation. Both callbacks may be `async`. Defaults: `pauseAfterDestroyMs: 250`, `maxChain: 32`. |
| `RunCascadeResult` | `{ chainLength, totalWinners, finalGrid, wasSkipped }` | Returned by `runCascade` AND delivered to `cascade:complete` listeners. `chainLength === 0` means no wins on the initial grid (no refill ran). |
