...
| `setResult` | `(symbols: string[][] \| ColumnTarget[]) => void` | Set the target grid. Triggers the stop sequence. Accepts the legacy `string[][]` form OR the explicit `ColumnTarget[]` form (preferred for grids that cross worker/JSON boundaries — negative-index slots don't survive `structuredClone`). |
...
| `skip` | `() => void` | Round-aware slam. First press boosts speed (standard mode) or flags refills to auto-slam (cascade mode); subsequent presses also slam. |
...
| `skipStage` | `readonly 0 \| 1 \| 2` | `0` until the first `skip()` press of the round, `2` after. Drive UI labels from this. |
...
| `runCascade` | `(opts: RunCascadeOptions) => Promise<RunCascadeResult>` | One-call cascade orchestration. Loops detect → destroy → pause → refill. Pass `signal: AbortSignal` for caller-driven cancellation (use this for "player tapped slam between refills" — `reelSet.skip()` is a no-op when the engine is idle). The same `mode` / `gravityHoldMs` / `gravityHold` / `onGravityComplete` options apply per chain (passed as `refillMode` + `gravityHold(info)` builder); see [Two-stage refill](#two-stage-refill). Per-stage progress fires via `cascade:chain:start` / `cascade:chain:end`; the awaited promise tells you when the chain itself is over. |
...
// between refills" (reelSet.skip() is a no-op when the engine is idle).
...
`runCascade` listens to `skip:requested` too — calling `reelSet.skip()` during an in-flight refill works as expected, but the AbortSignal pattern is the one that handles the idle-window case.
