...
| `runCascade` | `(opts: RunCascadeOptions) => Promise<RunCascadeResult>` | One-call cascade orchestration. Loops detect → destroy → pause → refill. Pass `signal: AbortSignal` for caller-driven cancellation (use this for "player tapped slam between refills" — `reelSet.skip()` is a no-op when the engine is idle). Per-stage progress fires via `cascade:chain:start` / `cascade:chain:end`; the awaited promise tells you when the chain itself is over. |
