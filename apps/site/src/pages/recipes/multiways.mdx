...
description: Per-spin row variation. Each reel lands on a different row count between minRows and maxRows. The reel pixel height stays fixed; cell heights are derived per spin. The Megaways formula.
tags: [multiways, megaways, shape, geometry, reshape]
steps:
  - "Build with .multiways({ minRows, maxRows, reelPixelHeight }) — defines the height budget"
  - "Each spin, pick a per-reel row count and call reelSet.setShape(rowsPerReel) BEFORE setResult()"
  - "AdjustPhase commits the new geometry; cell symbols snap, pin overlays tween"
apis:
...
realGameVideo:
  mp4: /videos/dog-house-megaways.mp4
  caption: "The Dog House Megaways by Pragmatic Play"
---
...
MultiWays is the engine surface behind every "Megaways"-style slot. The setup: a fixed pixel-height per reel column, a configurable `[minRows, maxRows]` range, and `setShape(rowsPerReel)` chosen per spin. The demo above runs **6 reels × 2-7 rows per reel** — exactly the Pragmatic Play "Megaways" footprint shown in the reference clip. Cell height is derived live (`reelPixelHeight / visibleRows[i]`), so a 2-row reel renders as two huge cells and a 7-row reel renders as seven small ones. Between SPIN and STOP the engine commits the new geometry per reel (resizes the strip's symbols, reshapes the motion layer) and then runs `AdjustPhase` to tween any pin overlays from their old cell to their new cell — the cell symbols themselves snap because they're still spinning at full speed.

The card-suit graphics in the demo are a no-asset prototyping default (`CardSymbol` from `examples/shared/CardSymbol.ts`) so each cell renders crisply at any size. In a real Megaways slot you'd ship `SpriteSymbol` / `AnimatedSpriteSymbol` / `SpineSymbol` — see [`card-symbol-debug`](/recipes/card-symbol-debug/) for when (not) to use the debug class.
...
- [MxN big symbols](/recipes/big-symbols-mxn/) — N×M blocks (mutually exclusive with MultiWays)
