...
title: Abort a nudge
description: Cancel a running nudge via AbortSignal. the strip still snaps to its deterministic landed position, but the `nudge()` promise rejects with `AbortError` and `nudge:cancelled` fires on the bus.
...
import code from '../../recipes/nudge-abort.recipe.ts?raw';
