...
For game code that crosses worker boundaries, use the `ColumnTarget[]` shape:
...
reelSet.setResult([
  ...
  { visible: result[1] },
  { visible: result[2] },
  { visible: result[3], bufferAbove: ['BONUS'] },
  { visible: result[4], bufferAbove: ['BONUS'] },
]);
```
...
- [Buffer indexing cheatsheet](/recipes/buffer-indexing-cheatsheet/) — full convention reference
