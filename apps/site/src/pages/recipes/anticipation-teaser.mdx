...
description: Pair setAnticipation with a buffer-above prefill so the next cell to scroll into view on a slow reel is a caller-chosen symbol.
...
steps:
  ...
  - During the anticipation decelerate, the buffer-above cell is the next to enter view
apis:
