...
description: The simplest thing — build your own symbol class that draws a coin with a number on it. Learn what a ReelSymbol is, the few methods the engine calls on it, and how to read the value back.
tags: [beginner, basics, symbol, coin, value, custom-symbol]
steps:
  - A symbol is any subclass of ReelSymbol; the engine makes one per cell
  - Take the value in the constructor and store it
  - Draw the coin + number in resize() (it runs on every swap with the cell size)
  - Read the value back off the landed symbol instance any time
apis:
  - ReelSymbol (subclass it)
  - ReelSetBuilder.symbols / register
  - Reel.getSymbolAt
---

import code from '../../recipes/coin-value-symbol.recipe.ts?raw';
