...
steps:
  ...
  - "Each spin, setShape(rowsPerReel) before setResult(). setShape migrates pins eagerly"
...
- **Reshape + AdjustPhase are multiways-only.** `pin:migrated`, `shape:changed`, and `adjust:*` events never fire on non-MultiWays slots.
