...
description: A 5x4 Hold & Win board where every cell is a real Spine skeleton, with MINI / MAJOR jackpot coins whose tier one-shot IS the reveal, side plaques that celebrate, and a collect flight into a Spine counter.
tags: [hold-and-win, spine, collect, respin, jackpot]
steps:
  - Register each id against its skeleton via a spineMap (the jackpot coin uses a small custom class for its tier reveal)
