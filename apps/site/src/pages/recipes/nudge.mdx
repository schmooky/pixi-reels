...
Each `await` parks the loop until that reel's GSAP tween lands. Reads as
three reveals — players can register reel 1's wild before reel 2 starts
moving. Total wall time = `cols.length * duration`. Use for scripted
reveals where each reel earns its own beat (jackpot teasers, "watch this
one too" moments, narrative bonus rounds).
...
Use for base-game responsiveness, line-completion reveals, and any time
the reveal IS the row landing as a unit.
