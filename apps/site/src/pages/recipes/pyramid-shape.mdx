...
new ReelSetBuilder()
  .reels(5)
  .visibleRowsPerReel([3, 5, 5, 5, 3])  // mutually exclusive with visibleSymbols(n)
...
That's it. `visibleRowsPerReel` and `visibleSymbols` are mutually exclusive — picking either is a one-line decision.
