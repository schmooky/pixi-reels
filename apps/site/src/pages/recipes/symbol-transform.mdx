...
async function transformCell(reelIndex: number, row: number, newSymbolId: string) {
  ...
  // Phase 2: swap identity. setSymbolAt is the one-cell rewrite. leaves
  // the rest of the strip alone, no buffer regen, no re-shuffle. Throws
  // mid-spin and on big-symbol anchors so it can't silently misbehave.
  reelSet.setSymbolAt(reelIndex, row, newSymbolId);
