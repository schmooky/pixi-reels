...
title: Cascade anticipation refill. gravity, hold, column wave
description: Split the cascade refill in two. Survivors slide down to fill the holes first, a global anticipation beat for multipliers and mascots, then new symbols drop in column by column. First-class on pixi-reels.
tags: [cascade, tumble, refill, two-stage, anticipation, recipes]
steps:
...
import anticipationCode from '../../recipes/tumble-anticipation.recipe.ts?raw';
...
<RecipeFrame>
  <RecipeRunner code={anticipationCode} client:only="react" />
</RecipeFrame>
