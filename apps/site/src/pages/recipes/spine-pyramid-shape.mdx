...
description: Per-reel static shape (3-5-5-5-3 pyramid) rendered with Spine skeletons. The outer 3-row reels get taller cells than the inner 5-row reels. Spine scales each rig cleanly without the texture stretching you'd see with sprites.
tags: [shape, pyramid, spine, custom-symbol, animation]
steps:
  - "`await loadGeneratedSpines()` once before building"
  - "Use `.visibleRowsPerReel([3, 5, 5, 5, 3])` to declare the jagged shape"
...
import code from '../../recipes/spine-pyramid-shape.recipe.ts?raw';
