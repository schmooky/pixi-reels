...
steps:
  ...
  - cascade:round:end tells you the chain ended
apis:
...
The library's `cascade:round:end` event fires once after `runCascade` exits, carrying `RunCascadeResult` — listen if you want a single hook for "the cascade chain is over" (big-win UI, autoplay continuation, analytics).
...
- **`reelSet.runCascade({ detectWinners, nextGrid })`** — the canonical detect → destroy → pause → refill orchestration with `cascade:round:start` / `cascade:round:end` framing the chain
