...
steps:
  ...
  - cascade:complete tells you the chain ended
apis:
...
The library's `cascade:complete` event fires once after `runCascade` exits — listen if you want a single hook for "the cascade chain is over" (big-win UI, autoplay continuation, analytics).
...
- **`reelSet.runCascade({ detectWinners, nextGrid })`** — the canonical detect → destroy → pause → refill orchestration with `cascade:complete` at the end
