...
**Asymmetry by design.** Down-nudges can land the block partially in
bufferBelow (the anchor stays in visible; the stub spills below the
mask — see the recipe). Up-nudges must keep the anchor IN visible:
the engine's `_finalizeFrame` doesn't size anchors that sit above
visible today, so allowing anchor → bufferAbove would render a broken
1x1 block. Filed as a follow-up; for now the engine fails loud rather
than rendering a half-broken state.

Blocks that wouldn't survive throw with a precise message naming the
