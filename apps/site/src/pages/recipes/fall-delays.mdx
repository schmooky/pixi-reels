...
title: Lead-in delay. how long before the fall starts
description: Four interactive canvases. Same spin, same destruction, same .tumble() feel. only the pause between SPIN click and the first frame of fall-out animation changes. From snappy 0 ms to dramatic 700 ms.
tags: [cascade, tumble, lead-in, pacing, recipes]
steps:
  - "Click SPIN. your handler runs immediately"
  - "Optionally await a lead-in pause (0 / 150 / 350 / 700 ms here)"
  - "Call reelSet.spin(). the fall begins"
apis:
  - ReelSet.spin
  - "(user-code: await new Promise((r) => setTimeout(r, ms)))"
---

import instantCode from '../../recipes/fall-delay-instant.recipe.ts?raw';
import subtleCode from '../../recipes/fall-delay-subtle.recipe.ts?raw';
import anticipationCode from '../../recipes/fall-delay-anticipation.recipe.ts?raw';
import suspenseCode from '../../recipes/fall-delay-suspense.recipe.ts?raw';
...
<RecipeFrame>
  <RecipeRunner code={instantCode} client:only="react" />
</RecipeFrame>
...
<RecipeFrame>
  <RecipeRunner code={subtleCode} client:only="react" />
</RecipeFrame>
...
<RecipeFrame>
  <RecipeRunner code={suspenseCode} client:only="react" />
</RecipeFrame>
