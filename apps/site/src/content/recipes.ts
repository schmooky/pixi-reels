...
export const RECIPES: RecipeMeta[] = [
  ...
  },
  {
    ...
    oneLiner: 'Cycle winning paylines with WinPresenter — lines, dim, events, default playWin().',
    steps: [
      'Build WinPresenter with a GraphicsLineRenderer',
      'After spin lands, await presenter.show(paylines)',
      'Abort on spin:start so slam-spins cancel in-flight presentation',
    ],
    apis: ['WinPresenter', 'GraphicsLineRenderer', 'win:* events'],
    tags: ['wins', 'paylines', 'presenter'],
  },
  {
    slug: 'paylines-custom-animation',
    title: 'Paylines: custom symbol animation',
    oneLiner: 'Replace playWin() with a GSAP timeline or a spine animation name, styled per payline.',
    steps: [
      'Pass symbolAnim as a callback or named animation',
      'Receive (symbol, cell, payline) — return a Promise',
      'Let WinPresenter handle lines, dim, events for you',
    ],
  ...
  },
  {
    ...
    steps: [
      'WinPresenter without a lineRenderer',
      'Subscribe to win:line / win:symbol',
      'Draw or trigger whatever you want with reelSet.getCellBounds',
    ],
    apis: ['ReelSet.getCellBounds', 'win:line', 'win:symbol'],
