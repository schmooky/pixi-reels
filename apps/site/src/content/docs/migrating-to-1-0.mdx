...
order: 4
description: Mechanical changes to move from 0.x to 1.0.0. Every breaking change
  with a before and after.
---

> **On v2?** This page covers the 0.x to 1.0.0 step. Its `After` snippets are
> written in v2 form so they still run today, but several names changed again
> in 2.0.0. Do this page's rewrites first, then
> [Migrating to 2.0](/docs/migrating-to-2-0/) -- or just run
> `npx pixi-reels-codemod v1-to-v2`, which handles the mechanical half.
...
The deprecated `visibleSymbols` builder setter is removed. It aliased `visibleRows`, which 2.0.0 renamed again to `visibleCells`. Replace every call:
...
```ts
builder.reels(5).visibleCells(3);
```
...
| Before | After |
|---|---|
| `Reel._getAnchorCell(visibleRow)` | `Reel.getAnchorCell(visibleCell)` |
...
- `setStopFrame`, `setCrossReelResolver`, `getAnchorCell`, `notifySpinStart`, `notifySpinEnd`, `notifyLanded`, `snapToGrid`, `reshape`.
...
It shares `finalGrid` and `wasSkipped` with `RunCascadeResult`; `winnersRefilled` and `duration` are refill-specific. one stage's worth.
...
```ts
const nudging = reelSet.nudge(2, { distance: 1, direction: 'forward', incoming: ['wild'] });
