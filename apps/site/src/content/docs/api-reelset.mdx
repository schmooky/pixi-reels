...
| `requestHurry` | `(options?: { speed?: string }) => void` | A press that lands instead of cutting. Frees the same reels a `requestSkip()` press would (tease protection, `'stepwise'`, reel groups) and asks each for its landing: the tease ends, the stop delay is cut, the spin-out and bounce play. Queues pre-`setResult` like `requestSkip()`. No boost, no `wasSkipped`, `skipStage` untouched, so a `requestSkip()` after it still slams whatever is still moving. `speed` names a registered profile the hurried stop lands on (spin-out speed and bounce); an unknown name throws. In cascade mode a tumble reel lands by placing, so the press slams and warns with code `hurry-cascade`. Fires `hurry:requested`. |
| `slamStop` | `(options?: { reels?: number[] \| except?: number[] }) => void` | Unconditional slam. No boost, no auto-slam flag, no tease protection. With `reels` / `except` (not both) it lands only those reels and every other reel keeps running to a natural landing. Called before `setResult()` it lands on whatever the strip is showing (random fill, or an invisible board in cascade mode) and warns with code `slam-before-result`; use `requestSkip()` there instead. |
...
| `skipStage` | `readonly 0 \| 1 \| 2` | `0` until the first `skipSpin()` press of the round, `2` after. `1` means a press landed the reels around a protected tease and left the tease running. the button should stay live, because the next press ends it. Drive UI labels from this, but gate on `isSpinning` first: the stage is ROUND-scoped and only resets on the next `spin()`, so an `'always'` round (where no press ever reaches `2`) leaves it parked at `1` through the idle window. |
