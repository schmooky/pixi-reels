...
| `setAnticipation` | `(reelIndices: number[], options?) => void` | Mark reels that should enter `ANTICIPATION` before stopping. Held indices are filtered silently. Options: `stagger`, `slowdown`, `duration`, and `protect` (`'once'` \| `'always'`) to stop a skip press hiding the tease. |
