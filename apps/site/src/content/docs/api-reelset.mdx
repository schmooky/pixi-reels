...
| `slamStop` | `(options?: { reels?: number[] \| except?: number[] }) => void` | Unconditional slam. No boost, no auto-slam flag, no tease protection. With `reels` / `except` (not both) it lands only those reels and every other reel keeps running to a natural landing. |
