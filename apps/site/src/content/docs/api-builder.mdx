...
| `.moves({ start, stop, anticipation })` | *(defaults)*. Replace the animated beats of the built-in phases without replacing the phases: `start.pull`, `start.accelerate`, `stop.bounce`, `anticipation.slowdown`. A move gets the reel, the profile, the numbers the default uses and an `AbortSignal`; returns a gsap animation, a promise or nothing. `null` removes a beat. See [Phases](/docs/api-phases/#moves). |
| `.skipMode('slam' \| 'quicken')` | `'slam'`. What a skip press does to the reels it frees when `requestSkip()` / `skipSpin()` do not say. See [ReelSet](/docs/api-reelset/). |
