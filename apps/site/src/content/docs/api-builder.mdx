...
| `.reelExtents([h,...])` | Per-reel pixel-box heights for pyramid layouts. Derived from `visibleCellsPerReel × symbolHeight` if unset. |
...
| `.multiways({ minCells, maxCells, reelExtent })` | *(unset; required for `setShape(...)`)* |
