...
Both emitters' `on(name, cb)` returns an `off()` function to unsubscribe. The library cleans up its own listeners on `destroy()`; a thrown exception inside a listener does not stop other listeners on the same event from running (but it is logged and may pollute your error console. handle your own errors).
...
| `anticipation:reelEnd` | `{ reelIndex, order, total }` | That reel's tease is over. |
...
| `cascade:fall:symbol` | `{ symbol: ReelSymbol, view: Container, reelIndex: number, cellIndex: number, duration: number, ease: string, distance: number }` | One symbol's fall tween is about to start. Read `symbol` / `view` to attach a parallel tween. |
...
| `cascade:dropIn:symbol` | `{ symbol: ReelSymbol, view: Container, reelIndex: number, cellIndex: number, duration: number, ease: string, offsetCells: number }` | One symbol's drop-in tween is about to start. `offsetCells` is the number of cells this symbol traverses (1 for top-row refills, more for survivors sliding past larger holes). |
...
| `cascade:gravity:symbol` | `{ symbol: ReelSymbol, view: Container, reelIndex: number, cellIndex: number, duration: number, ease: string, offsetCells: number }` | One survivor's settle tween about to start. Same payload shape as `dropIn:symbol`, scoped to survivors only. |
