...
| Skip / slam | `skip:requested` · `skip:completed` · `skip:boosted` · `hurry:requested` | `skipSpin()` · `requestSkip()` · `slamStop()` · `requestHurry()` |
...
| `hurry:requested` | `({ reels: number[] })` | A `requestHurry()` press freed these reels. Each lands through its stop rather than being placed, so its landing arrives as the usual `spin:reelLanding` then `spin:reelLanded`, a bounce apart. `skip:requested` still means "about to be placed" and never lists a hurried reel. |
...
it. Default `'info'` (everything) - see the [debugging guide](/guides/debugging/#console-notices)
for what each level covers and why the default is not quieter. In a browser a notice prints as a styled
...
| `hurry-cascade` | warn | `requestHurry()` in cascade mode. A tumble reel lands by placing its symbols, so there is nothing to spin out; the press slammed as `requestSkip()` would. |
