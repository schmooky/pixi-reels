...
order: 5
eyebrow: Reference
description: Plain-English definitions for every term you will see in pixi-reels
  docs, code, and slot-game conversations.
---
...
**Grid**. The two-dimensional snapshot of visible symbols, indexed `grid[reelIndex][cellIndex]`. A 5x3 slot has a 5x3 grid.
...
**Reel strip**. The (long, invisible) list of symbols a reel draws from. pixi-reels does not expose this directly. it asks for a next symbol from a frame middleware chain.
...
**`ReelSet`**. The top-level object, "the whole slot board": a PixiJS container that holds the reels, the spin controller, the speed manager, and the spotlight. You build one with `new ReelSetBuilder()...build()` and `addChild` it to your stage.
...
**`SpinningMode`**. The per-frame motion strategy: one method, `computeDelta(slotPitch, speed, deltaMs)`, returning how far the strip travels this frame. `StandardMode` is the default. `CascadeMode` multiplies by a gravity factor (1.5) and caps the step at one slot pitch. `ImmediateMode` returns 0, so the phase places symbols directly. Not the same thing as `spin({ mode: 'standard' | 'cascade' })`, which picks the phase chain.
...
**`FrameBuilder` / middleware**. A tiny middleware pipeline that fills a reel's whole frame -- buffer-start + visible cells + buffer-end -- with symbol identities each time the engine builds one. Intercept it to implement "no three-in-a-row", "inject a mystery symbol every 7 frames", etc.
...
**`CellPin`**. The cell-persistence primitive. Lock a symbol id to a cell with `reelSet.pin(reel, cell, id)`; the engine acquires an overlay during spin, migrates the cell on reshape, and supports an animated `movePin(from, to)` tween. Works with any `ReelSymbol` subclass. The pin primitive is general, not Spine-specific.
...
**`CheatEngine` (outside the library)**. Deterministic outcome generator used by the recipes and by the library's own integration tests. Lives in the private `@pixi-reels/cheats` workspace package, not in the library. ADR 009 covers why.
