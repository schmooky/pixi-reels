...
group: pyramid
oneLiner: Non-uniform reel set. 3-5-5-5-3 pyramid, diamond, half-pyramid. Static
  shape set at build time.
description: Build a non-uniform reel set. 3-5-5-5-3 pyramid, diamond,
  half-pyramid. Static shape, set at build time, no per-spin reshaping.
order: 3
steps:
  - Pass an array to .visibleRowsPerReel([3, 5, 5, 5, 3]) instead of
    .visibleRows(n)
  - Optionally set .reelAnchor("center" | "top" | "bottom") to control vertical
    alignment
