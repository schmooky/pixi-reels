...
group: wilds
oneLiner: Same sticky wild, built on the engine primitive. no ghost sprites, no
  manual grid injection.
description: Sticky wild using the engine's CellPin primitive. no ghost sprites,
  no manual grid injection. Pin once on land, the engine handles the rest.
order: 61
steps:
...
<RecipeDemo code="sticky-wild-pin" />
