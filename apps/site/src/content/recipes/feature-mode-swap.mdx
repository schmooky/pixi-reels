...
group: runtime
oneLiner: Enter and exit a bonus mode at runtime by toggling a frame middleware.
  zero rebuild.
description: Toggle a bonus mode on and off at runtime by adding and removing a
  frame middleware. no ReelSet rebuild, no rebuild latency.
order: 72
steps:
