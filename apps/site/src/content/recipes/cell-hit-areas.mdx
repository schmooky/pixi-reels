...
group: cell-coords
oneLiner: Attach pointer events to individual grid cells with getCellBounds.
  cursor turns to pointer on hover, click toggles a pick state.
description: Use getCellBounds to attach pointer events to individual grid
  cells. cursor changes to a pointer on hover, click toggles a pick state, all
  cells stay selectable across spins.
order: 70
steps:
  - For each cell, call reelSet.getCellBounds(col, row)
  - Build an invisible PIXI.Graphics rect with eventMode "static" + cursor
    "pointer"
