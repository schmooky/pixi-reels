...
title: MultiWays
group: multiways
oneLiner: Per-spin row variation. each reel can land on a different row count
  between minRows and maxRows.
description: Per-spin row variation. Each reel lands on a different row count
  between minRows and maxRows. The reel pixel height stays fixed; cell heights
  are derived per spin. Total ways = product of per-reel rows.
order: 5
steps:
