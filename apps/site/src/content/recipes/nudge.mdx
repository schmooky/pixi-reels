---
title: "Nudge"
group: mechanics
oneLiner: Shift a landed reel up or down by N positions to reveal new symbols, sequential, parallel, staggered, skippable or abortable.
order: 50
tags:
  - nudge
  - stagger
  - spotlight
  - big-symbols
  - abort
  - skip
apis: []
steps: []
---

Shift a landed reel up or down by N positions to reveal caller-supplied symbols. one primitive, with pacing and control variants below.

<a id="nudge"></a>

## Nudge a reel

Shift one reel after it lands; `incoming` is the final visible order, top-down. Below: the base nudge, then the same row of wilds dropped sequentially (three beats) vs in parallel (one beat).

<RecipeDemo code="nudge" height={300} />
<RecipeDemo code="nudge-sequential" height={300} />
<RecipeDemo code="nudge-parallel" height={300} />

<a id="nudge-stagger"></a>

## Staggered nudge. wave reveal

`Promise.all` with a per-reel `startDelay` fires every reel's tween concurrently for a wave reveal. validation still throws synchronously.

<RecipeDemo code="nudge-stagger" height={300} />

<a id="nudge-spotlight"></a>

## Spotlight after a nudge

Land a near-miss, parallel-nudge three reels to complete a wild line, then run `SymbolSpotlight` on the new winners.

<RecipeDemo code="nudge-spotlight" height={300} />

<a id="nudge-big-symbol"></a>

## Nudge through a big symbol

A 1xH block on the target reel is nudged through as a unit; the second demo drags a tail-visible block fully into view.

**Gotcha:** only single-reel (w=1) blocks can be nudged. cross-reel (w>1) blocks throw.

<RecipeDemo code="nudge-big-symbol" height={300} />
<RecipeDemo code="nudge-tail-reveal" height={300} />

<a id="nudge-abort"></a>

## Abort a nudge

Cancel a running nudge via `NudgeOptions.signal`. the strip still snaps to landed, but `nudge()` rejects with `AbortError` and `nudge:cancelled` fires.

<RecipeDemo code="nudge-abort" height={300} />

<a id="nudge-skip"></a>

## Skip an in-flight nudge

`reelSet.skipNudge(col)` fast-forwards a running nudge to its landed state; `nudge()` resolves normally, so the success path still runs.

<RecipeDemo code="nudge-skip" height={300} />
