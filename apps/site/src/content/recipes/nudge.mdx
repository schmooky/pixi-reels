...
multiple `nudge(...)` calls, and the orchestration is yours:
...
fail immediately); only the mutation is deferred by `startDelay`.
For irregular stagger profiles compute per-reel delays in the map.
