...
group: cell-coords
oneLiner: getCellBounds(col, row) returns the pixel rectangle of any visible
  cell. draw paylines, win outlines, or hit areas that align exactly with
  symbols.
description: getCellBounds(col, row) returns the pixel rectangle of any visible
  cell. Use it to draw payline lines, win-box outlines, or invisible hit areas
  that align exactly with symbols.
order: 69
steps:
  - Call reelSet.getCellBounds(col, row) to get { x, y, width, height } in
    ReelSet-local coords
