...
group: big-symbols
oneLiner: "A tall wild lands with its anchor in bufferAbove (tail-visible at row
  0). Nudge the trigger reel to drag the full block into view, then call `spin({
  holdReels: [...] })` so the revealed wild survives a respin of the other
  reels. The held strip is never touched."
description: A tall wild lands with its anchor in bufferAbove (tail visible at
  row 0). Nudge the trigger reel to drag the full block into view, then call
  spin with holdReels so the now-revealed wild survives a respin of the other
  reels.
order: 9
steps:
  - Land a 1xH wild with its anchor in `bufferAbove` on the trigger reel (only
    the bottom cell shows at row 0)
  - Nudge the trigger reel down to drag the full block into visibility
  - "Call `reelSet.spin({ holdReels: [triggerReel] })` to skip START/SPIN/STOP
    on that column"
  - Pass a full-width grid to `setResult`. The held column entry is ignored; the
    revealed block stays put
apis:
  - ReelSet.spin
...
<RecipeDemo code="big-symbol-held-respin" />

A tall wild lands with most of its body hidden above the visible
window. only the **tail** (the bottom cell) shows at row 0. The player
nudges to drag the whole wild into view, then gets a respin of the
OTHER reels with the wild reel held. the revealed block stays put.
This recipe wires the full sequence.

## Why this works end-to-end

Three pieces have to cooperate. the audit case for the
