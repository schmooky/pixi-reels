...
group: tension
oneLiner: A 1xH block on the target reel is nudged through as a unit. Survival
  formula enforced; cross-reel (w>1) blocks throw.
description: A 1xH block on the target reel is nudged through as a unit. The
  engine survival-checks anchor + h fits on the strip post-rotation; cross-reel
  (w>1) blocks still throw.
order: 59
steps:
  - Land with a 1x2 wild at rows 0+1
  - Nudge DOWN by 2. block survives (anchor + h + distance < total), lands
    half-visible
...
<RecipeDemo code="nudge-big-symbol" />
...
<RecipeDemo code="nudge-tail-reveal" />
