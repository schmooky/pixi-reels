...
group: features
oneLiner: A normal 5x3 spin that, on 3 BONUS, transitions in the same chain into
  the Hold & Win board, runs the feature, and swaps back to the base game on
  feature:end. Two display objects, one Spin button.
description: A normal 5x3 spin that, when 3 BONUS land, transitions in the same
  chain into the Hold & Win board — every cell now its own reel — runs the
  feature, and swaps back to the base game on feature:end. Two display objects,
  one Spin button.
order: 38
steps:
