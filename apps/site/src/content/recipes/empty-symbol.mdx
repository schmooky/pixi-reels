...
group: starters
oneLiner: Register an EmptySymbol against an id like `empty` to reserve a grid
  slot that produces no visual output. The canonical background for hold-and-win
  bonus boards, miss cells, and any pattern where "blank" is the intended
  visual.
description: Register an EmptySymbol against an id like 'empty' to reserve a
  grid slot that produces no visual output. Useful for hold-and-win mechanics,
  miss cells, and any board where blank is the intended visual.
order: 13
steps:
  - Import EmptySymbol from `pixi-reels`
  - Register the empty id via `r.register('empty', EmptySymbol, {})`
  - Weight empty heavily so most cells land blank; weight real symbols lightly
    so they pop
