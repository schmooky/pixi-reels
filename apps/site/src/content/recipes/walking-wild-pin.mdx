...
group: wilds
oneLiner: Walking wild migrating one column left each spin. engine-native
  reelSet.movePin(), no ghost sprites.
description: Walking wild migrating one column left each spin via
  reelSet.movePin(). no ghost sprites. With Spine-ready flight hooks for
...
<RecipeDemo code="walking-wild-pin" />
