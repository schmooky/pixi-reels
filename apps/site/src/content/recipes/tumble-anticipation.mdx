...
Same two-stage refill, one tuning change: `COLUMN_STEP_MS` raised past `dropIn.duration` (420 ms vs 380 ms), so each drop-in column fully lands before the next one starts. The anticipation stops being one global beat and starts *walking the board*. reel 0, breath, reel 1, breath, reel 2. Pair it with a longer `gravityHoldMs` (600 ms here). the walk earns the bigger stage:
