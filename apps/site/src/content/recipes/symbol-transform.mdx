...
group: features
oneLiner: A symbol morphs into a different (usually higher) one mid-round.
  winning lows upgrade to meds.
description: A symbol morphs into a different (usually higher-paying) one
  mid-round. after a win, on a timer, or when a condition is met. Common in
  cluster-pays and upgrade-style slots.
order: 43
steps:
