...
steps:
  ...
  - Present first. WinPresenter in runCascade's presentWinners hook
...
apis:
  - RunCascadeOptions.presentWinners
...
`WinPresenter` is the same presenter you use for paylines: a "win" is just cells, whether they came from a payline, a cluster, or a cascade pop. In `runCascade`'s **`presentWinners`** hook the presenter plays the winners' authored `win` clip while they are still on the board and losers dim; when it resolves, the library's `destroySymbols` chains the authored `explode`, and the refill follows:
