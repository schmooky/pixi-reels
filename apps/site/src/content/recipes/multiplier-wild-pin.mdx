...
group: wilds
oneLiner: Each wild carries a per-instance multiplier in its pin payload. a ×N
  badge overlays the cell.
description: Each wild carries a per-instance multiplier value in its pin
  payload. Game layer reads the payload during win evaluation to scale the
  payout.
order: 63
steps:
...
<RecipeDemo code="multiplier-wild-pin" />
