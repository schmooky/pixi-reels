...
group: cascade
oneLiner: Fade winners out, then drop the next-stage symbols in from above. no
  reel respin.
description: Fade the winners out, then drop the next-stage symbols in from
  above. a proper cascade with no reel respin.
order: 44
steps:
  - Detect winning cells on the visible grid
  - reelSet.destroySymbols(winners) fades them out
  - reelSet.refill({ winners, grid }) slides survivors and drops new symbols
    from above
apis:
