...
<a id="symbol-swap-reveal"></a>

## Reveal a mystery symbol: out, swap, in

`setSymbolAt` swaps a symbol's identity, but it swaps it *instantly*. So a
reveal - the mystery tiles dissolve, what is underneath changes, the prize
arrives - meant hand-rolling the same five things every time: the ordering, a
per-cell stagger on each side of it, a zIndex bump so an entrance that overshoots
its cell is not clipped by the neighbour drawn after it, a re-hide after the swap
(re-activation resets `alpha` to 1, so the new art pops for a frame before its
entrance begins), and abort handling so a player skipping the presentation still
ends up looking at what the server sent.

```ts
await reelSet.swapSymbols(
  [0, 1, 2].map((cell) => ({ reel: 1, cell, id: 'WILD' })),
  {
    outDelay: (_, i) => i * 0.06,   // peel the tiles away, top to bottom
    holdMs: 200,                    // the beat where the reveal sound goes
    inDelay: (_, i) => i * 0.09,    // the prize arrives, same order
  },
);
```

The three beats are separately skippable, because the middle one is the only one
the engine has to own. Art that plays its own Spine `out` track passes
`skipOut: true` and keeps the rest; `skipIn: true` leaves the swapped cells
hidden for you to reveal yourself. `onSwapped` is the hook for a midpoint beat
that has to wait on a real animation rather than a guessed number of
milliseconds - its promise is awaited.
...
being consumed" poof - a reveal is a different beat, and borrowing that one reads
wrong.

**Gotchas.** Cells are validated up front, so a bad coordinate fails before
anything has animated. An abort still performs the swap - the board must never be
left disagreeing with the result. Only valid at rest; `setSymbolAt` throws
mid-motion. Single-cell symbols only: a big symbol spans cells the frame layer
has to reserve, so revealing one is a `setResult` / `setShape` job.

<RecipeDemo code="symbol-swap-reveal" />
