...
group: basics
oneLiner: "Don't draw the coin — use art you already have (a sprite) and add the
  number on top. Your symbol's view holds two children: the art and a text
  label."
description: Don't draw the coin — use the artwork you already have (a sprite or
  Spine coin) and add the number on top. Your symbol's view holds two children,
  the art and a text label. The practical, production-style way.
order: 2
steps:
