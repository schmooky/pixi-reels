...
group: cascade
oneLiner: Four interactive canvases. same destruction, only the post-win refill
  ordering changes. Simultaneous, LTR wave, bottom-up, top-down.
description: Three interactive canvases. Same scripted win, same destruction,
  same .tumble() feel. only the cascade-refill ordering changes. See which one
  fits your slot's rhythm.
order: 19
steps:
...
Each canvas below runs the same scripted spin: a 6×4 board of real Thunderkick spine symbols lands left-to-right with a pre-wired 3-cluster of `low1` on row 2. The winners go through the same `reelSet.destroySymbols(...)` destruction, playing their authored `explode` clip. Only the **refill drop-in order** varies between canvases. controlled by `setDropOrder()` on the reel set and `rowStagger` / `rowOrder` on the builder's `.tumble({ dropIn })` config.
...
<RecipeDemo code="refill-simultaneous" />
...
<RecipeDemo code="refill-ltr-wave" />
...
// ...
reelSet.setDropOrder('ltr', F(6));  // 6 frames (100 ms) between reels, left to right
await reelSet.refill({ winners, grid: stage1.map((visible) => ({ visible })) });
```
...
<RecipeDemo code="refill-bottom-up" />

```ts
.tumble({
  dropIn: {
    rowStagger: F(6),          // 6 frames (100 ms)
