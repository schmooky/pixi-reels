...
group: features
oneLiner: Each coin's amount ticks up from 0.00 to its value as it settles, in a
  reading-order wave, instead of snapping to the number.
description: Each coin's amount ticks up from 0.00 to its value as it settles,
  in a left-to-right wave, instead of snapping to the number. The count is a
  plain gsap tween that rewrites the gold-font label every frame.
order: 34
steps:
