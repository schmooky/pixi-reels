...
2. `PAUSE_AFTER_REMOVAL_MS`. the breathing room between the explosion and the refill drop-in. Different feels want different pauses: snappy slams want ~120 ms, slow / dramatic feels want ~300-400 ms. Each canvas's value is tuned to match its tumble shape.
...
Gentle gravity fall + a clean decelerating settle. Start here; tweak from here. Note the ease family: these symbols have full-size rectangular plates, so overshoot eases (`back`, `bounce`) are out. an overshooting frame gaps from its neighbours and overlaps the row below. Overshoot only reads well on frameless art.

## Soft float. slow settle, heavy stagger
...
The slow, playful variant: a long 700 ms `power2.out` with a heavy stagger, so each row drifts in on its own beat. (On frameless art this slot is where you'd reach for `bounce.out`. framed plates can't bounce without the rectangles colliding.)
...
`power3.in` accelerates the fall steeply (heavy, gravity-like). `expo.out` lands hard with no overshoot. The whole tumble plays in under half a second. A reasonable basis for a turbo mode toggle on top of a default-classic build.
...
Heavy `rowStagger` (110 ms on drop-in) makes each row land in its own beat. the wave IS the stagger. Row 0 lands, then row 1, then row 2. Pair with row-by-row sound cues if you want a per-row audio hit. The more cascades in a chain, the more rows arrive in sequence.
...
| Soft float | 320 ms | Long, slow settle with heavy stagger | Turbo-first games. the float slows the perceived pace |
