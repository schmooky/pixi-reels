...
group: big-symbols
oneLiner: Square 2×2, tall 1×3, giant 3×3, wide 2×4. one focused interactive
  demo per shape, with the placement logic spelled out.
description: Big symbols at any rectangular size. 2×2 squares, 1×3 tall bars,
  3×3 giants, 2×4 wide banners. Same registration API for all four; each shape
  has its own use case. One interactive example per shape, with the placement
  logic spelled out.
order: 7
steps:
  - "Same registration API for every shape: { weight: 0, size: { w, h } }"
  - "Anchor formula: col in [0, REELS - w], row in [0, ROWS - h]"
  - getSymbolFootprint + getBlockBounds resolve any cell to its anchor + pixel
    rect
apis:
  - SymbolData.size
...
<RecipeDemo code="big-symbols-mxn-square" />
...
<RecipeDemo code="big-symbols-mxn-tall" />
...
<RecipeDemo code="big-symbols-mxn-giant" />
...
<RecipeDemo code="big-symbols-mxn-wide" />
