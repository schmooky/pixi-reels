...
Every canvas below runs the same scripted spin + destruction on real Thunderkick spine symbols (winners play their authored `explode` clip). Only the `LEAD_IN_MS` constant at the top of each recipe changes.
