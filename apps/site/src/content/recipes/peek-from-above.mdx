...
group: tension
oneLiner: Pre-fill the buffer-above slot so a teaser symbol sits just out of
  frame, visible during scroll-in on the next spin.
description: Pre-fill the buffer-above slot so a teaser symbol sits just out of
  frame. visible during scroll-in on the next spin.
order: 46
steps:
  - Seed initialFrame with a ColumnTarget that sets bufferAbove at build time
  - On every spin, pass the same { visible, bufferAbove } shape to setResult so
    the teaser carries forward
  - When the next spin starts, that symbol is the first to scroll into the
    visible area
apis:
...
<RecipeDemo code="peek-from-above" />
