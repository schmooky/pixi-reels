...
group: multiways
oneLiner: Pin survives every MultiWays reshape. clamps when shape shrinks,
  restores to originRow when it grows back.
description: Pin survives every MultiWays reshape. Wild stays at its originRow
  when the shape allows; clamps to the last row when the shape shrinks; migrates
  back when the shape grows again.
order: 15
steps:
  - Pin wilds on spin:allLanded. originRow is frozen at placement
  - Each spin, setShape(rowsPerReel). AdjustPhase migrates pins via
    min(originRow, newRows-1)
  - pin:migrated fires per affected pin with { fromRow, toRow, clamped,
    reelIndex }
apis:
  - ReelSet.pin
  - CellPin.originRow
...
<RecipeDemo code="sticky-wild-multiways" />
