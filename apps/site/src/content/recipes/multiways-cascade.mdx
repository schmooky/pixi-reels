...
The canvas above rolls its per-reel row counts uniformly, which mostly lands mid-range silhouettes. This one scripts the extremes. alternating comb shapes (`5-2-5-2-5-2`, its inverse, and a valley) so you can watch the two systems hold up when neighbouring reels differ by the full `minRows..maxRows` range: a 2-row reel cascades in the same phase as its 5-row neighbour, and the ways evaluation just sees per-reel arrays of different lengths:
