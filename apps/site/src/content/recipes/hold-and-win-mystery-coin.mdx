...
group: features
oneLiner: Coins land blank; on lock a strip of candidate amounts spins inside
  the coin face and decelerates onto the real value. The value lives in
  coin.data — the board never reads it.
description: A coin lands as a blank gold disc and its amount is unknown until,
  on lock, a strip of candidate values spins inside the coin face and
  decelerates onto the real number. The value lives in coin.data; the board
  never reads it.
order: 31
steps:
  - Coins land settled on the gold money face with no number painted yet
  - On coin:locked, scroll a masked strip of candidate amounts inside the coin
    and ease it onto the real value
