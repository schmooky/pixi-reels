...
Hold & Win art is usually wider than tall. cellSize takes a width and a height plus a columnGap and a rowGap, so the board lays out to the art's own rectangle, with the game's grid showing through the gaps.
...
## Collect: lightning strikes the values, the clover dims

Held clovers idle until the feature ends. When COLLECT locks, lightning strikes from it to each gold clover's number, the collect face counts the value in, and the struck clover dims to grey: still held, spent.
...
Real gaps between the cells (columnGap and rowGap) and no chrome, so the board is a grid the background shows through: a gradient panel and glowing grid lines, plain PIXI.Graphics drawn behind the board and seen only in the gaps.
