...
description: One atlas file, 80+ frames, zero per-image HTTP requests. Plus the
  separate-images fallback when you don't want to pack.
order: 60
steps:
  - Load the TexturePacker JSON with PIXI.Assets.load(). PixiJS understands the
    format natively
