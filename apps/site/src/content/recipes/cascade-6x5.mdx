...
These symbols have full-size rectangular plates, so every ease is
overshoot-free (no `back`/`bounce`. an overshooting frame gaps from its
neighbours; that family only reads well on frameless art):

| Feel | fall.ease | dropIn.ease | dropIn.rowStagger | notes |
|---|---|---|---|---|
| Classic | `power2.in` | `power3.out` | 50 ms | clean decelerating settle, all-rounder |
| Soft float | `power2.in` | `power2.out` (700 ms) | 70 ms | slow drift, each row its own beat |
| Slam stop | `power3.in` | `expo.out` | 25 ms | heavy, hard land |
| Rain column | `power2.in` | `power2.in` | 0 ms (+ `distance: 'auto'`) | whole column drops as a slab |
| Wave | `power2.in` | `power3.out` | 110 ms | rolling top-to-bottom arrival |
