...
group: big-symbols
oneLiner: Draw a single rectangle that hugs an entire N×M block, regardless of
  which cell you pass. Works for 1×1 (equivalent to getCellBounds) and 1×N
  expanding wilds.
description: Draw one overlay rectangle that hugs an entire N×M block,
  regardless of which cell you pass. Works for any size. 1×1 (same as
  getCellBounds), 1×N expanding wilds, 2×2 / 3×3 blocks.
order: 11
steps:
  - getBlockBounds(col, row) returns { x, y, width, height } in ReelSet-local
    pixels
...
<RecipeDemo code="get-block-bounds" />
...
`getBlockBounds` returns ReelSet-**local** coordinates: `(0, 0)` is the top-left of the ReelSet container, not the page. Add the overlay as a child of the ReelSet (`reelSet.addChild(overlay)`) so the math works. If you want page-space coordinates, transform with `reelSet.toGlobal({ x, y })`.
