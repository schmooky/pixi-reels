...
group: wilds
oneLiner: Wild lands, entire column becomes wild for N spins. auto-expires when
  the turn counter runs out.
description: When a wild lands, the whole column fills with wild and stays
  expanded for N spins. Built with CellPin numeric turns. auto-expires when the
  counter runs out.
order: 62
steps:
  - On spin:allLanded, find every reel containing a wild
  - "Pin every row of that reel with { turns: N }. the column stays wild for N
    more spins"
  - The engine decrements turns after each spin:allLanded and expires pins at
    zero
apis:
  - ReelSet.pin
...
<RecipeDemo code="expanding-wild-pin" />
