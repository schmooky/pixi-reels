...
`feature:end` flow still resolves, you stop waiting. `board.hurry()` is the
press that lands instead of cutting: every in-flight cell drops its spin floor
(the stagger lives there, so the wave lands together) and still spins its
symbol in and bounces; `board.hurry({ speed: 'turbo' })` lands it on turbo.
...
| `feature:hurry` | `{ inFlight }` | `hurry()` lands the in-flight cells through their stop |
...
its own buffers. The art the engine lifts above a cell's mask at rest (`unmask:
true`) renders in one layer above all cells, in attach order -- column-major --
...
`ReelSetBuilder`, applied inside every cell -- see [big symbols](/guides/big-symbols/#zindex-layering).
...
cuts the wave in progress - or `board.hurry({ speed: 'turbo' })`, which lands it on turbo
instead of cutting it. Each name also gets a `name:tension` variant the board uses for an
anticipating wave.
