...
That call shifts reel `2` forward by one cell. The old first cell drops to the
...
is always the bottommost. When `distance` exceeds `visibleCells`, the
...
Where `total = bufferStart + visibleCells + bufferEnd` and
`anchorStripIdx` is the anchor's index on the full strip (0 = topmost
bufferStart cell, `bufferStart` = visible row 0).

The block can land **partially** in either buffer and still render
correctly. `_finalizeFrame` scans both bufferStart and visible-row
...
wild lands with its anchor in bufferStart (tail visible at row 0);
...
```ts
// 1x2 wild lives in visible row 1 + bufferEnd. Nudge up by 1 reveals
