...
- **Use Spine symbols with custom disintegration**. your `SpineReelSymbol` subclass overrides `playDestroy()` to play the `out` animation; everything else stays the same. See [cascade wins. present, destroy, refill](/recipes/cascade-destroy-and-present/) for the spine wiring.
