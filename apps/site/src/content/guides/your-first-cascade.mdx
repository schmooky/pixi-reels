...
order: 2
eyebrow: Tutorial
description: From `npm install` to a working cascade slot in ~80 lines of
  TypeScript. No prior pixi-reels knowledge required.
---
...
enableDebug(reelSet);   // exposes window.__PIXI_REELS_DEBUG, used in Step 7
```
...
const server = {
  ...
  cascade: async (prev: string[][], winners: Cell[]) => {
    ...
    return prev.map((column, reel) => {
      ...
      if (!losers || losers.size === 0) return [...column];
      const survivors = column.filter((_, cell) => !losers.has(cell));
      const fillers = Array.from({ length: losers.size }, randSymbol);
      // Convention: top losers.size cells = new, rest = survivors in original order.
...
Game-rule code. left-anchored runs of 3+ matching symbols at the same cell index count as wins. `wild` matches anything.
...
async function playRound(signal: AbortSignal): Promise<void> {
  ...
  reelSet.setResult((await server.spin()).map((visible) => ({ visible })));
  await spinDone;
...
button.addEventListener('click', async () => {
  if (isPlaying) {
    reelSet.requestSkip();       // slam now, or queue it if the result hasn't landed
...
- `reelSet.requestSkip()` slams the engine's in-flight motion, and queues the
  intent if `setResult()` has not arrived yet. That window is real here: the
  mock server waits 400-1000 ms. Use `requestSkip()` rather than `skipSpin()`,
  which **throws** if it is called before the result lands -- there is nothing
  to slam onto yet. Both are no-ops when the engine is idle.
...
Combined: the engine slams its in-flight motion AND the cascade chain ends. See [the cancellation section in the cascades guide](/guides/cascades/#cancelling-a-chain-via-abortsignal) for the rationale.
...
Step 2 called `enableDebug(reelSet)`, so the browser console has
`window.__PIXI_REELS_DEBUG.log()`. It prints the visible grid and engine
state. Useful when something looks wrong.
...
- **No slam-state machine**. `skipSpin()` is round-aware; `wasSkipped` propagates through the chain.
...
- **Use Spine symbols with custom disintegration**. your `SpineReelSymbol` subclass overrides `playDestroy()` to play the `out` animation; everything else stays the same. See the [pyramid-cascade example](https://github.com/schmooky/pixi-reels/tree/main/examples/pyramid-cascade) for the spine wiring.
