...
order: 17
eyebrow: For authors
description: How to give a recipe a card thumbnail and inline body images.
  Convention-first — drop a file in public/ and it appears. Missing assets fall
  back to a generated placeholder, so the page always looks finished.
---

Every recipe card and every recipe page can carry preview media: a gif, an animated webp, a static screenshot, or a short clip. It is **convention-first and zero-config**: drop a file in the right place and it shows up. Nothing is required. anything missing renders a generated placeholder built from the recipe title, so the index and pages always look finished.
...
Inside a recipe's MDX body, use the `Image` component for figures (it is `RecipeImage.astro` under the hood): gifs of a mechanic mid-motion, before/after stills, a frame breakdown:
...
The Hold & Win recipes ([respin](/recipes/hold-and-win/), [Spine coins](/recipes/hold-and-win-spine/), [collector](/recipes/hold-and-win-collector/)) ship real captured cards; browse the rest of the [grid](/recipes/) to see the placeholders they fall back to.
