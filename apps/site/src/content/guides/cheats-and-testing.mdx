...
order: 15
eyebrow: For authors
description: Force deterministic outcomes, run spin lifecycles in Node, write
  mechanic tests that never flake.
---
...
`spinAndLand()` calls `spin() → setResult() → slamStop()`, which
bypasses all timing and resolves on a microtask. Perfect for CI.
(`slamStop()` lands every reel outright; it does not go through the
two-stage `skipSpin()` boost.)
...
The [Sprite classic](/demos/sprite-classic/) demo mounts a `CheatPanel`
that toggles these cheats at runtime: check a cheat → spin → watch the
advertised outcome land every time. The other demos declare cheats but do
not mount the panel.
