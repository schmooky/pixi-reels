...
`runCascade` is a one-screen wrapper around `refill`. Outgrow it and you will
lift the same five lines into your own handler. No magic to lose.
...
the server has not answered. Fill it however you like:

1. **Spinner overlay** -- show on fall end, hide on first drop-in. Cheap.
2. **Pulsing frame** -- nothing occludes the empty grid.
3. **Skeleton cells** -- placeholder squares.
4. **Nothing** -- under 100ms server time, nobody notices.
5. **Short fall** -- shrink `fall.duration` and let `dropIn` carry the energy.

## The pause matters

Put a beat between winners fading out and the refill starting.

Without it the refill begins on the frame the winners hit alpha 0, and the
player reads it as a teleport. Give it 150-500ms and the brain sees two
events: wins cleared, then new symbols arrived.
