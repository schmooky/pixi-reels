...
description: Each reel can have its own row count, cell height, and offset.
  Covers static pyramid layouts, the mask strategy that clips them cleanly, and
  how this interacts with MultiWays and big symbols (which have their own
  guides).
---
...
| **MultiWays** + Pyramid | **rejected** at build (`cannot combine multiways() with visibleCellsPerReel()`) |. |. |
...
- **MultiWays + pyramid**: `visibleCellsPerReel` declares a *static* shape; `multiways({...})` declares a *dynamic* shape. They contradict each other. both say "reel `i` has N rows."
...
new ReelSetBuilder()
  .reels(5)
  .visibleCellsPerReel([3, 5, 5, 5, 3])   // jagged shape
...
The engine computes a per-reel `mainOffset` so shorter reels are positioned according to `reelAnchor`. The default mask draws one clip rect per reel. short reels never leak buffer rows above or below their visible area.

`getCellBounds(col, row)` accounts for `mainOffset` automatically; `debugSnapshot.visibleCells` is `number[]` (one per reel) so jagged shapes are visible to debug tooling.
