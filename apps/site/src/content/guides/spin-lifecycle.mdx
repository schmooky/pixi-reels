...
order: 5
---
Unless you've changed it, your spin has 3 phases:
...
You *might* enable anticipation explicitly before setting the result, which makes that phase appear. Most games with scatters need it anyway, so the engine ships it ready to use.
...
`requestSkip()` is the "slam when ready" one. You can call it before `setResult()` arrives. No boost, no auto-slam. Queues until `setResult()`, then slams once. Use it when the player mashes the slam button before the result arrives.

`slamStop()` is an unconditional "land it now". No boost, no auto-slam.
...
The `skipStage` getter reports the round's stage (`0` before any press, `2` after). Use it to change the spin button's visuals.
...
`setAnticipation([...])` filters held indices silently.

## Overriding spin mode

Some slots spin the reels, then cascade after the initial spin. `SpinOptions.mode` overrides the builder's default phase chain per call.
