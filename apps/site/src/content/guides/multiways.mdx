...
description: Per-spin row variation. Each reel can land on a different row count
  between minCells and maxCells, with cell height derived from a fixed reel pixel
  height. The engine reshapes each reel between SPIN and STOP and AdjustPhase
  tweens any pin overlays; pins migrate via originCell.
---

MultiWays is the open-source name for the "ways"-style mechanic where each reel can land on a different row count *every spin*. Reel **pixel height is fixed** for the whole slot; cell height per reel is derived as `reelExtent / visibleCells[i]`. So a 2-row reel has tall cells; a 7-row reel has short ones.

For the broader picture (pyramid layouts, big symbols, mutual exclusivity rules), start at the [Per-reel geometry](/guides/per-reel-geometry/) overview.
...
| Spin | Shape (reel 2) | originCell | Migrated cell | Clamped? |
|------|---------------|-----------|--------------|----------|
| 1 | 5 | 4 | 4 | no |
| 2 | 3 | 4 | 2 | yes |
| 3 | 7 | 4 | 4 | **restored** |
...
The pin never restores after a clamp. `originCell` is updated on every clamp, so future grows see the new origin. Right for walking wilds, descending wilds, or anything where "current row" is the source of truth and a higher row is unreachable history.

| Spin | Shape (reel 2) | originCell before | Migrated cell | originCell after |
|------|---------------|------------------|--------------|-----------------|
| 1 | 5 | 4 | 4 | 4 |
...
- The reel pixel height is fixed per the `multiways({ reelExtent })` value. To change it, rebuild.
...
- [`sticky-wild-multiways`](/recipes/sticky-wild-multiways/) recipe. pin migration in action with `originCell`
