...
const hud = new PIXI.Text({
  text: '',
  style: { fontFamily: 'monospace', fontSize: 12, fill: 0xffcc44 },
});
...
return {
  ...
  onSpin: async () => {
    ...
    // Two scripted refills. Once the round-ending press has landed, the
    // engine auto-slams these with no animation at all - that is the cascade
    // form of the round side effect a protected press deferred.
    //
    ...
    // mispositioned. Change cell 0 on the WINNER reels only.
    const WINNERS = [0, 2, 4];
    for (let chain = 1; chain <= 2; chain++) {
      ...
      const next = reelSet.getVisibleGrid().map((col, reel) => ({
        visible: WINNERS.includes(reel) ? [rv(), col[1], col[2]] : [...col],
      }));
      await reelSet.refill({
        grid: next,
        winners: WINNERS.map((reel) => ({ reel, cell: 0 })),
      });
    }
