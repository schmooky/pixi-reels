...
//
// HURRY ON THE BOARD. `board.hurry()`.
//
// Every cell is its own 1x1 reel set, and the board folds its stagger into
// each cell's spin floor. `board.hurry()` asks every in-flight cell for its
// landing: the floor drops, so the whole wave lands together, and each cell
// still spins its symbol in and bounces. `board.skip()` places them instead.
// `{ speed: 'turbo' }` lands the wave on the turbo profile.
//
// `feature:hurry` reports how many cells were in flight, the counterpart of
// `feature:skip`; the landing flow (`cell:landed`, `coin:locked`,
// `respin:end`) is unchanged and there is nothing for the game layer to cut
// short. The coins are PhaseCardSymbol, so a spinning cell is blue and a
// landing one violet; a placed one goes straight to grey.
...
const board = new HoldAndWinBuilder()
  ...
  .initialSpeed('normal')
  // A slow diagonal wave, so a hurry has something to collapse.
...
const hud = new PIXI.Text({
  text: 'press spin, then tap again mid-wave to HURRY it',
...
// When the wave landed relative to the press: one number per hurried wave.
let pressedAt = 0;
let landedInWave = 0;
board.events.on('feature:hurry', ({ inFlight }) => {
  pressedAt = performance.now();
  landedInWave = 0;
  hud.text = `hurry: ${inFlight} cells landing through their stop on turbo`;
});
board.events.on('cell:landed', () => {
  if (!pressedAt) return;
  landedInWave += 1;
  hud.text = `hurry: ${landedInWave} landed, last one ${Math.round(performance.now() - pressedAt)} ms after the press`;
});
...
return {
  ...
  onSpin: async () => {
    ...
    board.reset();
    board.enter(SEED);
    hud.text = 'feature running: tap to hurry the wave';
    await sleep(300);
