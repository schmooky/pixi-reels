...
describe('BoardGrid render order', () => {
  it('draws every chrome under every reel and lifts unmasked views above all cells', () => {
    ...
    // 6 cells: chrome x6, reel set x6, the lifted layer, then the three
    // nodes `dim()` and `lift()` draw through - all empty until called.
    ...
    expect(kids[13].children).toHaveLength(0);
    expect((kids[15] as unknown as RenderLayer).renderLayerChildren).toHaveLength(0);
