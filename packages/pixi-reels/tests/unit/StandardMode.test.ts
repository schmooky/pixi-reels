/**
 * M5 - StandardMode.computeDelta must clamp displacement symmetrically.
...
describe('StandardMode.computeDelta', () => {
  const mode = new StandardMode();
  const H = 100;

  it('caps positive displacement at half a symbol', () => {
    expect(mode.computeDelta(H, 1000, 1000)).toBe(H / 2);
  });
  ...
  it('passes sub-cap displacements through unchanged', () => {
    expect(mode.computeDelta(H, 0.3, 16)).toBeCloseTo(0.48, 5);
    expect(mode.computeDelta(H, -0.3, 16)).toBeCloseTo(-0.48, 5);
  });
});
