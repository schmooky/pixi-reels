...
Add: `BoardGrid.dim({ except, amount })` / `HoldAndWinBoard.dim(...)` - push every cell but the named ones into the background for a beat, the partner of `lift()`. One rectangle per dimmed cell, drawn above the cells' lifted art and below anything lifted, so a lifted cell stays out in front of the dim. Only one dim at a time is meaningful, so a second call throws instead of silently replacing the first.
