...
- `ReelSymbol.playDestroy(opts?)` — `opts.direction: 1 | -1` for coherent rotation (e.g. `w.reel % 2 === 0 ? 1 : -1`) and `opts.delay: number` (seconds) for per-winner stagger. Default direction stays random for back-compat.
